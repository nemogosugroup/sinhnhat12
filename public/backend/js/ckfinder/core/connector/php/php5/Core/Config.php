...
class CKFinder_Connector_Core_Config
{
	...
	*/
    ...
    public function &getResourceTypeConfig($resourceTypeName)
    {
        ...
        //while (list($_key,$_resourceTypeNode) = each($GLOBALS['config']['ResourceType'])) { // Lỗi chạy ở php7.2
        foreach($GLOBALS['config']['ResourceType'] as $_key => $_resourceTypeNode){ // Sửa thành như này
